...
def bfs_matt(graph, current_room):
    queue = Queue()
        ...
        if vert not in visited:
            ...
            if "?" in graph[vert].values():
                ...
                for room in path:
                    ...
                    room_dirs = list(graph[cur_room].keys())
                        
                    for d in room_dirs:
                            
                        if graph[cur_room][d] == next_room:
                                path_dirs.append(d)
                                cur_idx += 1
                                next_idx += 1
                                break
            ...
            for neighbor in graph[vert].values(): #self.get_neighbors(vert)
...
def bfs_neigh(neigh_graph, start_id,end_id):
    queue = Queue()
    ...
    while queue.size > 0:
        ...
        for neigh_id in neigh_graph[room_id].values():
            new_path = list(path)
